...
function minToDate(min, date) {
  ...
  var offset = dt.getTime() + min * 60 * 1000; //, 'ms'//.hour(0).minute(totalMins + min).second(0).millisecond(0);

  var offsetDate = new Date(offset);
  var result = new Date(date);
  result = _dates2.default.hours(result, 0);
  result = _dates2.default.minutes(result, totalMins + offsetDate.getHours() * 60 + offsetDate.getMinutes());
  result = _dates2.default.seconds(result, 0);
  result = _dates2.default.milliseconds(result, 0);
  console.log(result);
  return result;
}
